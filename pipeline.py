...
# 分区闸门：命中联名信号 → 归「🧋 奶茶联名」区；
# 未命中（只命中买一送一/免费等硬羊毛动作）→ 归「🥤 奶茶饮品」区。
# 两者同时命中时归联动区（口径：联动专门放联动区）。
#
# 注意：命中本闸门即**直接收录**（联名豁免抽奖闸门），所以误伤代价是把无关帖
# 塞进联名区 —— 补词必须保守，宁可少收不可错收。
#
# 「A × B」是官微写联名的另一种常见形态（「茶百道 ×《天官赐福》」「奈雪× @明日方舟」），
# 正文未必出现「联名」二字。约束：× 两侧须各有一个中文/字母/书名号/@ 字符 ——
# 既保证连的是「名字」而非数字，又排除了「2×3」「500ml×2」这类规格与乘法。
# 刻意不用裸 x：会误伤 iPhone x / XXL 之类。
# 左侧：中文/字母（品牌名）。
# 右侧：**刻意收紧**为书名号/@/字母 —— 联名的 × 后面跟的是「作品名或账号」
# （茶百道 ×《天官赐福》、奈雪× @明日方舟），带书名号或 @ 是强特征。
# 早前右侧也放行普通中文，结果把原料配比误收进来：
#   古茗「福建单芽×广西横州伏花茉莉 HPP冷压」—— 那是配料表不是联名。
# 代价：「茶百道 × 天官赐福」这种不带书名号的写法会漏，但宁可少收不可错收。
_LINK_LEFT = r"[\u4e00-\u9fffA-Za-z]"
_LINK_RIGHT = r"[《「【@A-Za-z]"
MILKTEA_X = re.compile(_LINK_LEFT + r"\s*[×✕]\s*" + _LINK_RIGHT)
# 刻意**不收**「携手」「合作」「联合」这类裸词：官微公益文案里高频出现
# （「携手XX基金会」「联合公益行动」），它们不是联名，收进来只会给联名区灌水。
...
def _milktea_verdict(txt, deal_re, lottery_re):
    ...
    if link_m:
        ...
        hit = link_m.group(0)
        # 「A × B」命中时 group(0) 是「雪× @」这种残缺片段，直接显示看不出所以然，
        # 统一归一化为「联名×」。
        if "×" in hit or "✕" in hit:
            hit = "联名×"
